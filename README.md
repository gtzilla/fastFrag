...
* Does fastFrag Execute JavaScript code?
    ...
    Let's looks at two examples, already in fastFrag Structure. In the first sample, we have added a string with script tags right in it. This will add a div to the page with exact text: "<script>alert('pwned');</script>". The script will not be executed.  (Hilariously, github markdown cannot safely display the string in the above, while fastFrag could. Reprinted as **code** below)
